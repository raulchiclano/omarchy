...
Item {
  ...
  // Match the rounded calendar and volume cards.
  ...
  readonly property int popupMargin: Style.space(16)
  readonly property int barClearance: liveBarSize + popupMargin
  ...
  // Local plugins cannot traverse the private lock service. Query only its
  // public status, and fail closed on missing, invalid or stale answers.
  property bool screenStateClear: false
  property double screenStateCheckedAt: 0
  readonly property bool screenObscured: !screenStateClear

  Process {
    id: screenStateProbe
    command: ["bash", decodeURIComponent(Qt.resolvedUrl("ScreenState.sh").toString().replace(/^file:\/\//, ""))]
    stdout: StdioCollector {
      id: screenStateOutput
      waitForEnd: true
    }
    onExited: function(exitCode) {
      service.screenStateClear = screenStateTimer.running && exitCode === 0
        && String(screenStateOutput.text || "").trim() === "clear"
      service.screenStateCheckedAt = Date.now()
    }
  }
  ...
  Timer {
    ...
    onRunningChanged: if (!running) service.screenStateClear = false
    onTriggered: {
      if (Date.now() - service.screenStateCheckedAt > 1000)
        service.screenStateClear = false
      if (!screenStateProbe.running) screenStateProbe.running = true
    }
  }
  ...
  // User preference: every toast lasts five seconds of visible reading time.
  // The existing popup timer pauses on hover or while the screen is obscured.
  ...
  Variants {
    ...
    PanelWindow {
      ...
      readonly property var popupPlacement: NotificationLogic.popupPlacement(
        service.barPosition, service.barClearance, service.popupMargin)
