...
Panel {
  ...
  readonly property color foreground: "#F2EFFA"
  readonly property color accent: "#B4A1F5"
  readonly property color urgent: bar ? bar.urgent : Color.urgent
  readonly property color dim: "#C9C0D8"
  readonly property color surface: "#F0211E2C"
  readonly property color track: "#403A50"
  ...
  function windowTitle(label) {
    var text = String(label || "").toLowerCase()
    if (text.indexOf("month") >= 0) return "Mensual"
    if (windowIsLong(text)) return "Semanal"
    if (text.indexOf("session") >= 0 || windowSpanMs(label) > 0) return "Sesión"
    var plain = String(label || "").replace(/\s*\(.*\)\s*/, "").trim()
    return plain === "" ? "Límite" : plain
  }
  ...
  function balanceDetailText(b) {
    ...
    var text = formatMoney(b.spent, b.currency) + " gastados de " + formatMoney(b.funded, b.currency) + " disponibles"
    if (b.estimated) text += " · estimado"
    return text
  }
  ...
  function heroMeta(p) {
    ...
    var tier = String(p.tierLabel || "").replace(/_/g, " ")
    if (tier === "") return "Suscripción"
    return tier.charAt(0).toUpperCase() + tier.slice(1)
  }

  // Local calendar date, recomputed from nowMs so a panel left open across
  // midnight moves the "Hoy" row with the clock.
  function todayDate() {
  ...
  function dayName(date) {
    ...
    return ["Dom", "Lun", "Mar", "Mié", "Jue", "Vie", "Sáb"][parsed.getDay()]
  }
  ...
  function dayTooltip(day, today) {
    ...
    if (today && provider && provider.hasPromptStats !== false)
      text += " · " + Number(provider.todayPrompts || 0) + " prompts · "
        + Number(provider.todaySessions || 0) + " sesiones"
    return text
  }
  ...
  function modelTooltip(row) {
    if (!row) return ""
    return "Entrada " + usage.formatTokenCount(row.input)
      + " · salida " + usage.formatTokenCount(row.output)
      + " · lectura de caché " + usage.formatTokenCount(row.cacheRead)
      + " · escritura de caché " + usage.formatTokenCount(row.cacheWrite)
  }
  ...
  function footerText() {
    ...
    if (provider && provider.syncEnabled && provider.syncDeviceCount > 0)
      return "Combinado de " + provider.syncDeviceCount + " dispositivo" + (provider.syncDeviceCount === 1 ? "" : "s")
    return ""
  }
      ...
      Flickable {
        ...
        Column {
          ...
          Text {
            ...
            text: "No se han encontrado suscripciones de IA.\nAparecerán aquí cuando las hayas utilizado."
            color: root.dim
  ...
  component LimitRow: Column {
    ...
    Item {
      ...
      Text {
        ...
        text: limitRow.window ? ({ "Weekly": "Semanal", "Monthly": "Mensual", "Session": "Sesión", "Limit": "Límite" }[limitRow.window.title] || limitRow.window.title) : ""
        color: root.foreground
        font.family: root.fontFamily
        font.pixelSize: Style.space(13)
        elide: Text.ElideRight
    ...
    Text {
      ...
      text: {
        ...
        return remainingMs > 0 ? "Se restablece en " + root.formatDuration(remainingMs) : ""
      }
      color: root.dim
      font.family: root.fontFamily
      font.pixelSize: Style.space(12)
  ...
  component DayRow: Item {
    ...
    Rectangle {
      ...
      Rectangle {
        ...
        color: dayRow.today ? root.accent : root.alpha(root.accent, 0.55)
