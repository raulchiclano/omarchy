...
import "file:///usr/share/omarchy/shell/services" as ShellServices
...
Item {
  ...
  // Some shell builds do not retain the scoped application-library proxy.
  // Reuse the public AppLibrary component locally when that proxy is absent.
  readonly property var appLibrary: root.shell && root.shell.appLibrary
    ? root.shell.appLibrary : localAppLibrary
  ShellServices.AppLibrary {
    id: localAppLibrary
  ...
  property color selectedBackground: "#33B4A1F5"
    ...
    BorderSurface {
      ...
      Item {
        ...
        ConfirmDialog {
          ...
          z: 10
          message: "¿Quieres desinstalar " + ((root.deleteTarget && root.deleteTarget.label) || "") + "?"
      ...
      Column {
        ...
        Item {
          ...
          ListView {
            ...
            delegate: BorderSurface {
              ...
              Text {
                ...
                font.family: row.iconFont.length > 0 ? row.iconFont : Style.font.family
          ...
          Column {
            ...
            Text {
              textFormat: Text.PlainText
              text: root.filterText ? "Sin resultados para «" + root.filterText + "»" : "No hay opciones aquí"
              color: root.foreground
              opacity: 0.7
              font.family: root.fontFamily
